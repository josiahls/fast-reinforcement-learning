...
def learner2gif(lnr:DDPGLearner,s_format,group_interp:GroupAgentInterpretation,name:str):
...
def test_ddpg_fit(model_cls, s_format, mem, env):
	...
	# exploration_method=OrnsteinUhlenbeck(size=data.action.taken_action.shape, epsilon_start=1, epsilon_end=0.1,
	# 	decay=0.001)
	# lnr=ddpg_learner(data=data, model=model, memory=memory, exploration_method=exploration_method,
	# 	callback_fns=[RewardMetric, EpsilonMetric])
	# lnr.fit(2)
	...
	del learner
	# assert config_env_expectations[env]['action_shape']==(1, lnr.data.action.taken_action.shape[1])
	# if s_format==FEED_TYPE_STATE:
...
def test_ddpg_models_pendulum(model_cls, s_format, experience):
	...
	for i in range(5):
		...
		learner2gif(learner,s_format,group_interp,'pendulum')
...
def test_ddpg_models_walker(model_cls, s_format, experience):
	...
	for i in range(5):
		...
		learner2gif(learner,s_format,group_interp,'walker2d')
...
def test_ddpg_models_ant(model_cls, s_format, experience):
	...
	for i in range(5):
		print('\n')
		data=MDPDataBunch.from_env('AntPyBulletEnv-v0', render='rgb_array', bs=64, add_valid=False, keep_env_open=False,
			feed_type=s_format, memory_management_strategy='k_partitions_top', k=3)
		exploration_method=OrnsteinUhlenbeck(size=data.action.taken_action.shape, epsilon_start=1, epsilon_end=0.1,
			decay=0.00001)
		memory=experience(memory_size=1000000, reduce_ram=True)
