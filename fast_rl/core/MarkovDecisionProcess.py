...
class Action(object):
    ...
    def get_single_action(self):
        ...
        if len(self.bounds) != 1: return self.taken_action.detach().numpy()[0]
        else: return self.taken_action.detach().numpy()[0][0]
...
class MDPCallback(LearnerCallback):
    def __init__(self, learn):
        ...
        self.valid_ds: MDPDataset = learn.data.valid_ds
    ...
    def on_epoch_end(self, last_metrics, epoch, **kwargs: Any) -> None:
        ...
        if last_metrics[0] is not None: self.valid_ds.x.set_recent_run_episode(epoch)
...
class MDPDataBunch(DataBunch):
    ...
    def from_env(cls, env_name='CartPole-v1', max_steps=None, render='rgb_array', bs: int = 64,
                 feed_type=FEED_TYPE_STATE, num_workers: int = 0, memory_management_strategy='k_partitions_both',
                 split_env_init=False, device: torch.device = None, no_check: bool = False,
                 add_valid=True, **dl_kwargs):

        env = gym.make(env_name)
        train_list = MDPDataset(env, max_steps=max_steps, render=render, bs=bs,
                                memory_manager=memory_management_strategy)
        if add_valid:
            valid_list = MDPDataset(env if split_env_init else gym.make(env_name), max_steps=max_steps,
                                    render=render, bs=bs, memory_manager=memory_management_strategy)
        else:
            valid_list = None
...
class MDPList(ItemList):
    ...
    def set_recent_run_episode(self, episode):
        print('set_recent_run_episode_num: I should be setting the episode num but I am not.')

    def clean(self, episode): [item.clean() for item in self.items if item.episode == episode]

    def _update_info(self, ep, item: MDPStep):
        self.info[ep] = float(np.sum(self.info[ep] + float(item.reward))) if ep in self.info else float(item.reward)
